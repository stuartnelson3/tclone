...
describe TweetParser do
  let(:tweet_parser) { TweetParser.new("Mention @user and @another_user and @user4") }

  context "parsing a tweet" do
    it "should pull out user_names" do
      expect(tweet_parser.parse_user_names).to eq(["@user", "@another_user", "@user4"])
    end
    ...
    it "should return the text with links to the usernames" do
      parsed_tweet = "Mention <a href='/user' class='mention-link'>@user</a> and <a href='/another_user' class='mention-link'>@another_user</a> and <a href='/user4' class='mention-link'>@user4</a>"
